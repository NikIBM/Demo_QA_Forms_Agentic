...
export function generateSummaryReport(resultsDir: string, totalElapsedTime?: number): void {
                    ...
                    for (const spec of suite.specs) {
                      if (spec.tests && spec.tests.length > 0) {
                        ...
                        for (const test of spec.tests) {
                          let testStatus: 'passed' | 'failed' | 'skipped' | 'timedout' = 'passed';
                          let testDuration = 0;
...
function generateHTML(stats: SummaryStats, results: TestResult[], resultsDir: string): string {
